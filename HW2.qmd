...
st_abbrev, cnty_name, region, acstotpop, lowincome, p_pm25, p_lifeexppct)
...
low_income <- joined %>% 
  ...
  summarize(low_inc_sum = sum(lowincome)) %>% #Get the sum of lowincome per grade, create object
  ungroup() %>% #Ungroup so we can access the total sum of lowincome
  #Add a column that uses sum per grade and total sum to yield a percent
  mutate(low_inc_pct = (low_inc_sum/sum(joined$lowincome)) *100) 
  

plot1 <- ggplot(low_income, aes(x = grade, y = low_inc_pct)) +
  geom_bar(stat = 'identity', fill = '#CE5425') +
  labs(title = "Percentage of Low Income Homes by HOLC Grade, LA County",
       x = "HOLC Grade",
       y = "% Homes that are Low Income") +
  theme_light()

pct_pm25 <- joined %>% 
  group_by(grade) %>% #Group by grade
  summarize(pm25 = mean(p_pm25))

plot2 <- ggplot(pct_pm25, aes(x = grade, y = pm25)) +
  geom_bar(stat = 'identity', fill = '#5E899E') +
  labs(title = "Mean Percentile PM25 by HOLC Grade", x = "HOLC Grade",
...
```{r}
ejscreen_raw <- st_read(here::here('data/ejscreen/EJSCREEN_2023_BG_StatePct_with_AS_CNMI_GU_VI.gdb')) %>% 
  clean_names() %>% 
  # Filter to LA County
  filter((st_abbrev == 'CA') & (cnty_name == 'Los Angeles County'))
```
